<?php

class Leadtech_City_Block_Adminhtml_City_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form
{
  protected function _prepareForm()
  {
      $form = new Varien_Data_Form();
      $this->setForm($form);
      $fieldset = $form->addFieldset('city_form', array('legend'=>Mage::helper('city')->__('Item information')));
     
      $fieldset->addField('title', 'text', array(
          'label'     => Mage::helper('city')->__('Title'),
          'class'     => 'required-entry',
          'required'  => true,
          'name'      => 'title',
      ));

      $fieldset->addField('filename', 'file', array(
          'label'     => Mage::helper('city')->__('File'),
          'required'  => false,
          'name'      => 'filename',
	  ));
		
      $fieldset->addField('status', 'select', array(
          'label'     => Mage::helper('city')->__('Status'),
          'name'      => 'status',
          'values'    => array(
              array(
                  'value'     => 1,
                  'label'     => Mage::helper('city')->__('Enabled'),
              ),

              array(
                  'value'     => 2,
                  'label'     => Mage::helper('city')->__('Disabled'),
              ),
          ),
      ));
     
      $fieldset->addField('content', 'editor', array(
          'name'      => 'content',
          'label'     => Mage::helper('city')->__('Content'),
          'title'     => Mage::helper('city')->__('Content'),
          'style'     => 'width:700px; height:500px;',
          'wysiwyg'   => false,
          'required'  => true,
      ));
     
      if ( Mage::getSingleton('adminhtml/session')->getCityData() )
      {
          $form->setValues(Mage::getSingleton('adminhtml/session')->getCityData());
          Mage::getSingleton('adminhtml/session')->setCityData(null);
      } elseif ( Mage::registry('city_data') ) {
          $form->setValues(Mage::registry('city_data')->getData());
      }
      return parent::_prepareForm();
  }
}